...
import { deletePortfolioCategory } from 'services'
...
interface IModalDeletePortfolioCategory {
  portfolioCategory: PortfolioCategory
  children: (props: TModalReturn) => JSX.Element
}

const ModalDeletePortfolioCategory = ({
  portfolioCategory,
  children,
}: IModalDeletePortfolioCategory) => {
  ...
  const handleDelete = async () => {
    setIsLoading(true)
    const response = await deletePortfolioCategory(token, portfolioCategory.id)
    response?.status == 202
...
export default ModalDeletePortfolioCategory
