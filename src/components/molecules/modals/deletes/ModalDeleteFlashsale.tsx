...
import { deleteFlashsale } from 'services'
import { flashsale } from 'types'
import { useRouter } from 'next/router'

type TModalReturn = { openModal: () => void }

interface IModalDeleteFlashsale {
  flashsale: flashsale
  children: (props: TModalReturn) => JSX.Element
}

const ModalDeleteFlashsale: FC<IModalDeleteFlashsale> = ({
  flashsale,
  children,
}) => {
  ...
  const handleDelete = async () => {
    setIsLoading(true)
    const response = await deleteFlashsale(token, flashsale.id)
    response?.status == 202
...
export default ModalDeleteFlashsale
