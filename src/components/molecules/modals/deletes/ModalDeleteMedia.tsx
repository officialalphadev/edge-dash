...
const ModalDeleteMedia = ({
  ...
  const handleDelete = async () => {
    setIsLoading(true)
    const responses = selectedMedias.map(async (media) => {
      return await deleteMedia(token, media.id)
    })
    await Promise.all(responses)
      .then(() => toast({ status: 'success', message: 'Berhasil hapus data.' }))
      .catch(() => toast({ status: 'error', message: 'Berhasil hapus data.' }))
    setIsLoading(false)
    onChange()
    setIsOpen(false)
    push({})
