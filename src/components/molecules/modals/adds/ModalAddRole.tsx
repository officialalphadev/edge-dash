...
const ModalAddRole = ({ children }: IModalAddRole) => {
  const initialState = { name: '', description: '', permissions: '' }
  ...
  const handleAddRole = async (event: SyntheticEvent) => {
    event.preventDefault()
    setIsLoading(true)
    const response = await createRole(token, dataPost)
    response?.status == 201
  ...
  return (
    <>
      ...
      >
        <form onSubmit={handleAddRole} className="scrollbar overflow-y-auto">
          <div className="sticky top-0 flex items-center space-x-3 border-b border-gray-200 bg-white px-5 py-3">
            ...
            <div className="flex flex-col space-y-1">
              <p className="font-semibold text-gray-900">Tambah Role</p>
              <p className="text-xs text-gray-500">
                Menambahkan role baru pada Dashboard Alphadev
              </p>
            </div>
          </div>
